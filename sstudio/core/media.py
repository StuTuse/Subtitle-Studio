...
def _extract_with_pyav(path: str, out_wav: str, sr: int,
    ...
    total = probe(path).duration or 0.0
    with av.open(path) as c, wave.open(out_wav, "wb") as wf:
        wf.setnchannels(1)
        wf.setsampwidth(2)
        wf.setframerate(sr)
        ...
        for frame in c.decode(stream):
            ...
            if progress and total:
                pos = float(frame.pts * frame.time_base) if frame.pts else 0.0
...
def default_wav_path(video_path: str) -> str:
    ...
    try:
        ...
        os.makedirs(d, exist_ok=True)
    return os.path.join(d, f"{stem}_{abs(hash(video_path)) % 9973}.wav")
